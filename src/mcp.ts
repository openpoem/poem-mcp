...
Use this when someone shares a .poem file or asks about its contents.`,
    inputSchema: {
      ...
      properties: {
        source: { type: 'string' as const, description: 'The .poem file content' },
        filename: { type: 'string' as const, description: 'Optional filename for context' },
      },
      required: ['source'],
    },
  },
...
Returns errors (must fix), warnings (should fix), and stats.`,
    inputSchema: {
      type: 'object' as const,
      properties: {
        source: { type: 'string' as const, description: 'The .poem file content to validate' },
      },
      required: ['source'],
    },
  },
...
Supported targets: any programming language (python, typescript, go, rust, java, sql, swift, kotlin, etc).`,
    inputSchema: {
      ...
      properties: {
        source: { type: 'string' as const, description: 'The .poem file content' },
        target: { type: 'string' as const, description: 'Target language (e.g. "python", "go", "typescript")' },
        filename: { type: 'string' as const, description: 'Optional source filename' },
        style: {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      switch (name) {
        ...
        case 'poem_validate': {
          const result = validatePoemFile(args?.source as string);
          return {
            content: [{
              type: 'text',
              text: JSON.stringify({
        ...
        case 'poem_translate': {
          ...
          const style = (args?.style as string) || 'idiomatic';
 ...
 */
function getPoemMapping(target: string): Record<string, string> {
